...
class SpotdlWrapper:
    ...
    def __call__(self, url:str) -> None:
        ...
        if url.find("youtube") != -1:
            platform = "youtube"
            playlist_name = self.get_youtube_playlist_name(url)

        elif url.find("spotify") != -1:
            platform = "spotify"
            playlist_name = self.get_spotify_playlist_name(url)

        skip_album_art = "--skip-album-art" if self.skipAlbumArt else ""
        print_errors = "--print-errors" if self.printErrors else ""

        clean_url: str = self.clean_url(url)
    ...
    def get_youtube_playlist_name(self, url: str) -> str | None:
        '''
    ...
    def get_spotify_playlist_name(self, url: str) -> str | None:
        """
    ...
    def get_playlist_urls(self,count: int) -> list[str]:
        urls = []
